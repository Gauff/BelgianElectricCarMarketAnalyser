...
request_url = (
    f"{site_root_url}/fr/lst?atype=C&cy=B&damaged_listing=exclude&desc=0"
...
def get_cars_from_web_site():
    ...
    while True:
        try:
            ...
            logger.error(f"Error while scraping page {page}: {e!s}", exc_info=True)
            consecutive_errors += 1
            if consecutive_errors >= max_consecutive_errors:
                logger.error(
                    f"Max consecutive errors ({max_consecutive_errors}) reached. Stopping."
...
def get_cars_from_last_file():
    ...
    try:
        json_file_path = file_management.get_last_generated_file_path(
            AUTOSCOUT24_RESULTS, file_name, ".json"
        )
...
def _create_driver():
    ...
    try:
        ...
        logger.error(f"Error creating Chrome driver: {e!s}", exc_info=True)
        raise
...
def _scrape_page(page):
    ...
    try:
        ...
        logger.error(
            f"WebDriver error while scraping page {page}: {e!s}", exc_info=True
        )
        return None
    except Exception as e:
        logger.error(f"Error while scraping page {page}: {e!s}", exc_info=True)
        ...
        if driver:
            try:
                ...
                logger.error(f"Error closing WebDriver: {e!s}", exc_info=True)
    ...
    for result in listing_json_data:
        try:
            car = ElectricCar(
                id=result["id"],
                brand_name=(
                    result["vehicle"]["make"].upper()
                    if result["vehicle"]["make"]
                    else "UNKNOWN"
                ),
                model_name=(
                    result["vehicle"]["model"].upper()
                    if result["vehicle"]["model"]
                    else "UNKNOWN"
                ),
                version=(
                    result["vehicle"]["modelVersionInput"].upper()
                    if result["vehicle"]["modelVersionInput"]
                    else "UNKNOWN"
                ),
                ...
                first_registration_year=utilities.extract_year(
                    result["tracking"]["firstRegistration"]
                ),
...
def _save_and_return(all_cars):
    ...
    try:
        ...
        json_file_path = file_management.save_json(
            all_cars_json, AUTOSCOUT24_RESULTS, file_name, ".json"
        )
