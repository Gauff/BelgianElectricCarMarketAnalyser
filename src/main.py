from dash import exceptions, html, Dash, callback_context, dcc, no_update
from dash.dependencies import Input, Output
import plotly.express as px
import webbrowser
import visualization
from data_preparation import get_cars, clean_car_list, filter_cars, prepare_dataset_for_display, save_dataframe
import graph_utils
from src import file_management, data_preparation
...
def generate_graph(df_cars):
    ...
    fig = px.box(df_cars, x='Price', y='model', points='all',
                 custom_data=['URL', 'Image URL', 'desc', 'Brand Name', 'Model Name', 'year', 'Kilometers', 'Price'],
                 color='year', color_discrete_sequence=color_scale,
                 height=plot_height)
    fig.update_layout(yaxis={'categoryorder': 'category descending'})
    fig.update_layout(legend=dict(title='Year', orientation='v', yanchor='top', y=1.0,
                                xanchor='left', x=1.02,
                                traceorder='normal'))
...
def scrap_ads():
    ...
    cars = sorted(cars, key=lambda car: (
    visualization._get_sort_key(car.brand_name, car.model_name), visualization._get_registration_year(car)))
    df_cars = prepare_dataset_for_display(cars)

    file_path = save_dataframe(df_cars)
...
def run_server():
    ...
    app.layout = html.Div([
        ...
        dcc.Graph(id="graph-id", figure=generate_graph(filtered_df_cars), clear_on_unhover=True),
        dcc.Interval(
    ...
    def display_hover(hover_data):
        ...
        children = [
            html.Div([
                dcc.Markdown(f"**{make} {model}**<br>{year} | {km}<br>**{price}€**", dangerously_allow_html=True),
                html.Img(src=image_url, style={"max-width": "300px", "max-height": "300px"}),
    ...
    def update_output(value):
        ...
        filtered_df_cars = df_cars[(df_cars['Price'] >= lower_price) & (df_cars['Price'] <= upper_price)]
